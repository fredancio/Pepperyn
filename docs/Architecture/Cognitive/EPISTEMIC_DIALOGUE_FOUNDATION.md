...
- **Confirms** the proposed interpretation → the Candidate (held ephemerally by Epistemic Dialogue, from FRU's Inference tier — **corrected 2026-08-09**: never itself written to `KnowledgeModel`, per the final contract's CONFIRMED-only persistence model) causes exactly one new CONFIRMED `KnowledgeModel` row to be inserted, at the scope the question was actually asked at (never wider — §7).
- **Corrects** the proposed interpretation → a *different* CONFIRMED row is inserted via `KnowledgeModel`'s supersession mechanism (`relates_to_knowledge_id`); the prior CONFIRMED row is never deleted or rewritten, only superseded (mirrors the One New Truth Rule's own pattern: freeze the old as a historical projection, don't merge it into the new). **Corrected 2026-08-09**: the rejected Candidate itself was never persisted in the first place (it existed only ephemerally in this dialogue turn), so there is nothing "superseded-Candidate" to retain — what persists is the prior CONFIRMED row, unchanged, still independently queryable. Adversarial review (AI reliability persona) flagged one distinction this document deliberately leaves open rather than resolving here: a correction can mean either "this organisation is a genuine, legitimate exception" or "the deterministic detector itself is systematically wrong." Distinguishing these — e.g. via correction-rate monitoring across organisations — is a real future concern, explicitly out of scope for this foundation and for the first vertical slice.
...
**Principle, not schema** (per the mission's own explicit instruction not to design the schema here): every Confirmed Context entry must carry an **explicit, recorded scope descriptor**, never an implicit or assumed one, and that scope must default to the *narrowest defensible boundary the evidence actually supports* — widening only through repeated, consistent confirming observation, borrowing `BusinessHistory`'s own INV-HISTORY-1 discipline (never promote confidence, here specifically scope-breadth, from a single occurrence). The realistic minimum boundary is **Entity** — the true owner per `KNOWLEDGE_MODEL_V0_IMPLEMENTATION_CONTRACT.md`'s final arbitration (canonical since `3be19e4`; **corrected 2026-08-09**, this section previously said "Entity/Engagement" following `COGNITIVE_CAPABILITY_MAP.md`'s now-superseded "propriété d'Engagement") — never global across organisations, never silently assumed workbook-wide when a sheet-level or template-level distinction is what the evidence actually supports. Engagement remains recorded as acquisition context (which mandate was active when the knowledge was learned) but is not itself a scope boundary.
...
| **Enterprise Understanding Memory** | "How should we interpret this organisation and its representation?" | **This is `KnowledgeModel`, already named in `COGNITIVE_CAPABILITY_MAP.md` — not a new memory family. Owned by Entity, not Engagement** (`KNOWLEDGE_MODEL_V0_IMPLEMENTATION_CONTRACT.md` final arbitration, canonical since `3be19e4`; **corrected 2026-08-09** — this row previously repeated `COGNITIVE_CAPABILITY_MAP.md`'s now-superseded "owned by Engagement"). | Epistemic Dialogue's Understanding and Contradiction Dialogue types are the mechanism that produces a CONFIRMED `KnowledgeModel` row. **Corrected 2026-08-09:** there is no persisted Candidate→Confirmed promotion inside `KnowledgeModel` itself — the final contract established that `KnowledgeModel` stores CONFIRMED knowledge only; a Candidate/hypothesis remains entirely ephemeral, held by Epistemic Dialogue (as a `ClarificationNeed` or equivalent runtime object) until and unless a human confirmation causes exactly one fully-formed CONFIRMED row to be inserted. This is precisely the operational gap `COGNITIVE_CAPABILITY_MAP.md` already named without designing ("LLM interdit for the final decision... requires human confirmation or explicit deterministic rule") — Epistemic Dialogue is the design for that missing mechanism, not a new persisted store next to it. |
