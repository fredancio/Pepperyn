...
| **Engagement** | ACQUISITION CONTEXT only for the resulting knowledge, not its owner. **Corrected 2026-08-09** per `KNOWLEDGE_MODEL_V0_IMPLEMENTATION_CONTRACT.md` (final arbitration, canonical since `3be19e4`): the true owner of `KnowledgeModel` rows is Entity (`entity_id`, CASCADE), not Engagement — a prior wording here and in `COGNITIVE_CAPABILITY_MAP.md` said "propriété d'Engagement," which the arbitration overturned (Engagement is `engagement_id`, SET NULL, acquisition context only, so that a second Engagement on the same Entity still benefits from what a prior one learned). |
