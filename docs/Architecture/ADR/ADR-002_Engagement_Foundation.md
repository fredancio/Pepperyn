...
- **La relation Entity:Engagement est 1:1, et le reste durablement — décision explicite, pas une simplification provisoire pour T2** (amendement suite à revue ; options écartées détaillées en §3.14). `entity_id` porte une contrainte `UNIQUE` sur `Engagement`, permanente, pas seulement « à la création ».
