...
### 2.7 Epistemic Dialogue v0 — retargeting d'une ClarificationNeed périmée en cas de re-RECALL
- **Nature :** dette de suivi issue de la revue adversariale indépendante pré-fusion d'Epistemic Dialogue v0 (2026-08-09), défaut architectural latent, pas un défaut corrigible par une simple extension de champ à ce stade.
- **Source :** revue adversariale indépendante « EPISTEMIC DIALOGUE v0 — INDEPENDENT ADVERSARIAL PRE-MERGE REVIEW » (2026-08-09), section 9 (CORRECT-TO semantics) ; confirmée par mission de corrections « EPISTEMIC DIALOGUE v0 — FINAL CORRECTIONS BEFORE MERGE » (2026-08-09, correction 1 traitée, ce point explicitement exclu de la correction sur instruction de Fred).
- **État réel :** `resolve_clarification()` (`backend/services/epistemic_dialogue_service.py`) ne conserve dans `ClarificationNeed` que `recalled_value` (une valeur), jamais l'identifiant de la ligne `KnowledgeModel` recalée au moment de la question (choix délibéré de minimalité, contrat §4). Pour superseder correctement, `resolve_clarification()` effectue donc un second `recall()` frais au moment de la résolution, pour retrouver la ligne prédécesseur réelle. Si la tête de chaîne canonique a changé entre le premier `recall()` (celui qui a produit la question posée à l'humain) et ce second `recall()` (au moment où l'humain répond), la réponse de l'humain est silencieusement retargetée pour superseder la ligne ACTUELLE, pas celle qu'il a réellement vue et évaluée. Dans le registre à deux valeurs légales de `EXPENSE_SIGN_CONVENTION` (v0's seul sujet réel), ceci ne peut produire qu'un maillon de chaîne redondant de même valeur, jamais une valeur économique finale erronée — donc non bloquant pour v0. Le risque devient réel dès qu'un futur sujet possède 3 valeurs légales ou plus : une réponse humaine pourrait alors se retrouver appliquée pour superseder une ligne dont la valeur diffère de celle que l'humain a réellement évaluée, sans qu'il en soit informé.
- **Pourquoi différé :** corriger ce point maintenant exigerait soit d'élargir `ClarificationNeed` (violerait la minimalité à 4 champs démontrée par le contrat §4, sans qu'aucun cas adversarial réel de v0 — sujet unique, 2 valeurs — ne le justifie, Article IX), soit de faire comparer `resolve_clarification()` la valeur re-recalée au `recalled_value` original et déclencher une ré-évaluation complète (COMPARE) plutôt qu'une résolution directe — un changement de flux réel, pas une petite correction, explicitement exclu du périmètre de la mission de corrections en cours par instruction de Fred.
- **Dépendances réelles :** aucune technique bloquante — dépend uniquement de l'apparition d'un premier sujet `SUBJECT_VALUE_REGISTRY` avec 3 valeurs légales ou plus, ou d'un volume d'usage concurrent suffisant pour rendre la fenêtre de course réaliste plutôt que théorique.
- **Ce qu'il bloque :** rien aujourd'hui — v0 a un seul sujet, 2 valeurs légales, et ce chemin n'a jamais été observé en pratique (fenêtre de course étroite : deux `ClarificationNeed` concurrentes sur le même `(entity_id, subject)`, résolues dans un ordre entrelacé précis).
- **Déclencheur de réouverture :** la conception du premier sujet `EpistemicDialogue`/`KnowledgeModel` avec 3 valeurs légales ou plus, OU toute décision produit d'élargir Epistemic Dialogue au-delà d'un seul sujet à 2 valeurs.
- **Action requise à ce moment-là :** à ce moment, comparer explicitement la valeur re-recalée au moment de `resolve_clarification()` avec `need.recalled_value` d'origine ; si elles diffèrent, ne pas superseder directement — réinvoquer `reason_recall_compare()` avec la nouvelle valeur recalée et traiter la réponse humaine comme s'appliquant à cette nouvelle comparaison (probable nouvelle Case C), plutôt que de superseder aveuglément la tête de chaîne actuelle. Ceci est une direction nommée par la revue, **pas une autorisation d'implémentation**.
- **Risque trop tôt :** élargir `ClarificationNeed` ou le flux de résolution pour un cas qu'aucun sujet réel ne déclenche encore — sur-ingénierie spéculative, contraire à la discipline de minimalité déjà appliquée avec succès au contrat Epistemic Dialogue v0 lui-même.
- **Risque trop tard :** le jour où un sujet à 3+ valeurs légales existe, ce défaut pourrait produire une supersession silencieusement incorrecte (l'humain confirme une chose, une autre est enregistrée comme si elle avait été confirmée par lui) sans qu'aucun signal n'alerte personne — contrairement au cas à 2 valeurs, ce ne serait plus un maillon redondant inoffensif mais une fausse attribution de confirmation humaine.
- **Ordre relatif :** aucun — attend son déclencheur, pas une place dans la séquence actuelle.
