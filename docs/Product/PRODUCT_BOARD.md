...
**Decision Memory Integrity Repair** (branche `implementation/decision-memory-integrity-repair-2026-08-08`) — DecisionArc ↔ Engagement fusionné (2026-08-08, `8db6070`) a mis au jour deux défauts d'intégrité préexistants, non introduits par cet incrément, tous deux confirmés par du code réel et non de simples hypothèses : (A) `decision_arcs.entity_id` n'est jamais peuplé par le chemin de création réel — vérifié empiriquement comme produisant zéro carte Portfolio Intelligence ; (B) `origin_analysis_id` est déclaré `NOT NULL` et `ON DELETE SET NULL` simultanément — combinaison contradictoire bloquant la suppression ordinaire d'une Analysis référencée (`DELETE /api/analyses/history`). Traités comme deux défauts indépendants à l'intérieur du même incrément, pour éviter qu'une correction serve de justification à l'autre. Une fois ce rattachement de mémoire décisionnelle terminé et revu adversarialement, le FTE devient l'étape suivante naturelle (donner une dimension temporelle à une mémoire relationnelle enfin propre : Organisation → Engagement → Analysis → DecisionArc). Justification de l'exclusivité : toute autre ouverture de chantier reste bloquée tant que Gate A-C (`PRE_IMPLEMENTATION_GATE_CHECKLIST.md`) ne sont pas passés.
...
**Une seule capacité ne peut jamais apparaître simultanément en section 1 et en section 4** — test de cohérence structurel : Portfolio Intelligence, T1C-A, T1C-B, T2A, Evidence Ledger Consumer #1 et DecisionArc ↔ Engagement (section 1) n'apparaissent nulle part dans le registre de chantiers différés pour eux-mêmes ; seuls leurs chantiers de suivi (cardinalité Entity:Engagement §1.2.a, résolution Evidence/DecisionArc §1.2.b, sémantique des issues de capture Evidence §1.3.a, provenance Evidence au niveau assertion §1.3.b, défauts entity_id/origin_analysis_id §1.6, FTE, Familiarization) y apparaissent, chacun avec son propre déclencheur ou, pour §1.6, en réparation active.
