...
- **T1C-A (Evidence Ledger — capture)** — ADR-001/001A ACCEPTED, fusionné dans `main` le 2026-08-07 (commit `3b1b21a`, revue adversariale passée, verdict A — MERGE AS-IS).
...
- **T2 (Engagement — T2A)** — ADR-002 ACCEPTED, code réel sur branche, extraction ciblée requise (jamais fusion de branche entière), en cours de reconstruction chirurgicale.
...
**Une seule capacité ne peut jamais apparaître simultanément en section 1 et en section 4** — test de cohérence structurel : Portfolio Intelligence, T1C-A et T1C-B (section 1) n'apparaissent nulle part dans le registre de chantiers différés ; T2 (section 2) apparaît bien dans le registre différé pour tout ce qui le suit (Engagement, FTE), jamais pour lui-même puisqu'il est ACCEPTED BUT NOT MERGED, pas différé.
