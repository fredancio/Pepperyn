...
**Aucun chantier d'implémentation actif.** Avec Decision Memory Integrity Repair (section 1), la mémoire décisionnelle de Pepperyn est stabilisée : Evidence (ce qui est su) → Engagement (la relation continue) → DecisionArc (ce qui a été recommandé, décidé, exécuté, appris), avec une continuité désormais garantie structurellement, pas seulement par convention. Fred a explicitement choisi une pause volontaire avant tout code FTE (Financial Time Engine) : définir d'abord, ensemble, ce que le temps financier signifie pour un CFO, avant d'autoriser la moindre implémentation — pour éviter de poser une couche temporelle propre sur un modèle relationnel qui aurait été encore ambigu. Le prochain chantier ne s'ouvre pas tant que ce cadrage n'a pas eu lieu.
...
**Une seule capacité ne peut jamais apparaître simultanément en section 1 et en section 4** — test de cohérence structurel : Portfolio Intelligence, T1C-A, T1C-B, T2A, Evidence Ledger Consumer #1, DecisionArc ↔ Engagement et Decision Memory Integrity Repair (section 1) n'apparaissent nulle part dans le registre de chantiers différés pour eux-mêmes ; seuls leurs chantiers de suivi (cardinalité Entity:Engagement §1.2.a, résolution Evidence/DecisionArc §1.2.b, sémantique des issues de capture Evidence §1.3.a, provenance Evidence au niveau assertion §1.3.b, interaction FK/trigger entity_id/engagement_id sur arc CLOSED §1.6.c, FTE, Familiarization) y apparaissent, chacun avec son propre déclencheur.
