...
/**
 * Page de checkout direct pour le plan PRO.
 * Lancée après login depuis le flow d'inscription PRO.
 * — Si pas de session : redirige vers /login?redirect=/checkout/pro
  ...
  useEffect(() => {
    async function startCheckout() {
      try {
        ...
        if (!session) {
          window.location.href = '/login?redirect=/checkout/pro';
          return;
        }
  ...
  return (
    <div className="min-h-screen bg-[#EFF6FF] flex flex-col items-center justify-center px-4">
      <div className="bg-white rounded-2xl shadow-xl p-8 border border-gray-100 max-w-md w-full text-center">
        ...
        <h2 className="text-xl font-bold text-[#1A1A2E] mb-2">Préparation de votre paiement…</h2>
