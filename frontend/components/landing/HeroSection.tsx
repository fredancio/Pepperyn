...
function WaterfallChart() {
  const bars = [
    { label: 'EBITDA\nInitial',  value: '2,1M€',  h: 80, color: '#1B73E8', positive: true  },
    { label: 'Achats',           value: '-0,6M€', h: 27, color: '#e07b2a', positive: false },
    { label: 'Frais fixes',      value: '-0,4M€', h: 19, color: '#e07b2a', positive: false },
    { label: 'Sous-\ntraitance', value: '-0,3M€', h: 14, color: '#e87070', positive: false },
    { label: 'Autres',           value: '-0,2M€', h: 10, color: '#DC2626', positive: false },
    { label: 'EBITDA\nCible',    value: '2,6M€',  h: 96, color: '#16a34a', positive: true  },
  ];
  return (
    <div className="flex items-end gap-1 h-28 px-0.5">
      {bars.map((b, i) => (
        <div key={i} className="flex flex-col items-center gap-0.5 flex-1">
          <span
            className="text-[6.5px] font-bold leading-tight text-center"
            style={{ color: b.color }}
          >
            {b.value}
          </span>
          <div
            ...
            style={{ height: b.h * 0.78, background: b.color, opacity: b.positive ? 1 : 0.85 }}
          />
          <span className="text-[5.5px] text-gray-400 text-center whitespace-pre-line leading-tight mt-0.5">
            {b.label}
          </span>
        </div>
      ))}
...
function Timeline() {
  return (
    <div className="relative flex items-center justify-between px-2 mt-1">
      {/* Background line */}
      <div className="absolute left-4 right-4 top-2.5 h-0.5 bg-gray-200" />
      {/* Active segment (dot 1 → dot 2) */}
      <div className="absolute left-4 right-1/3 top-2.5 h-0.5 bg-[#1B73E8]" />

      {[
        { days: '30 JOURS', label: 'Lancer',    outer: 'bg-[#1B73E8] border-[#1B73E8]', inner: 'bg-white' },
        { days: '60 JOURS', label: 'Accélérer', outer: 'bg-[#4E95EF] border-[#4E95EF]', inner: 'bg-white' },
        { days: '90 JOURS', label: 'Mesurer',   outer: 'bg-white border-[#16a34a]',      inner: 'bg-[#16a34a]' },
      ].map((s, i) => (
        <div key={i} className="flex flex-col items-center gap-1 z-10">
          <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${s.outer}`}>
            <span className={`w-2 h-2 rounded-full ${s.inner}`} />
          </div>
          <span className="text-[6px] font-bold text-gray-400 tracking-widest">{s.days}</span>
...
export function HeroSection() {
  return (
    <section className="relative bg-white overflow-hidden">
      ...
      {/* Subtle glow — upper right only, behind the mockup */}
      <div
        ...
        style={{
          ...
          width: '640px',
          height: '420px',
          background: 'radial-gradient(ellipse at 70% 30%, rgba(27,115,232,0.09) 0%, transparent 70%)',
          borderRadius: '50%',
        }}
      />
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-14 lg:pt-16 pb-0">
          ...
          <div className="relative flex items-start justify-center lg:justify-end">
            <div
              ...
              style={{
                ...
                boxShadow:
                  '0 4px 6px -1px rgba(0,0,0,0.04), 0 20px 60px -8px rgba(27,115,232,0.16), 0 8px 24px -4px rgba(0,0,0,0.07)',
            ...
            >
              ...
              <div className="grid grid-cols-2 divide-x divide-gray-100">
                ...
                <div className="p-4 flex flex-col gap-2.5">
                  <p className="text-[7.5px] font-bold tracking-widest text-[#1B73E8]" style={{ textTransform: 'uppercase' }}>
                    Décision prioritaire
                  </p>
                  <p className="text-[13px] font-bold italic text-[#1A1A2E] leading-tight">
                    Réduire les achats indirects
                  </p>

                  {/* Impact + ROI — plain text, no colored backgrounds */}
                  <div className="flex gap-5">
                    <div>
                      <p className="text-[7px] text-gray-400 leading-tight mb-0.5">Impact annuel estimé</p>
                      <p className="text-[19px] font-extrabold text-green-600 leading-none">+480 K€</p>
                    </div>
                    <div>
                      <p className="text-[7px] text-gray-400 leading-tight mb-1">ROI</p>
                      <div className="flex gap-0.5">
                        {[1, 2, 3, 4, 5].map(i => (
                          <svg key={i} className="w-3 h-3 text-amber-400 flex-shrink-0" viewBox="0 0 20 20" fill="currentColor">
                  ...
                  {/* MARGE / EBITDA / CASH with sparklines */}
                  <div className="grid grid-cols-3 gap-1 pt-2 border-t border-gray-100">
                    {[
                      ...
                      <div key={m.label} className="flex flex-col gap-0.5">
                        <p className="text-[6.5px] font-bold text-gray-400 tracking-widest">{m.label}</p>
                ...
                {/* RIGHT PANEL: 5 Décisions + Plan 90 jours */}
                <div className="p-4 flex flex-col gap-2.5">
                  <p className="text-[7.5px] font-bold tracking-widest text-gray-400" style={{ textTransform: 'uppercase' }}>
                    5 Décisions prioritaires
                  </p>
                  <WaterfallChart />

                  <div className="border-t border-gray-100 pt-2.5">
                    <p className="text-[7.5px] font-bold tracking-widest text-gray-400 mb-2" style={{ textTransform: 'uppercase' }}>
                      Plan 90 jours
                    </p>
                    <Timeline />
                  </div>
              ...
              <div className="px-4 py-3 bg-gray-50/80 border-t border-gray-100">
                <p className="text-[9px] font-semibold italic text-gray-600 mb-2">Recommandations clés</p>
                <div className="grid grid-cols-2 gap-x-4 gap-y-1.5">
                  {[
                    ...
                    <div key={i} className="flex items-center gap-1.5">
                      ...
                      <span className="text-[8.5px] text-gray-600 leading-tight">{rec}</span>
                    </div>
                  ))}
                </div>
              </div>
        ...
        {/* Trust line below badges */}
        <p className="text-center text-xs text-gray-400 pb-8 -mt-4">
