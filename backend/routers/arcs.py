...
async def get_portfolio(
    ...
    try:
        legacy_cards = arc_service.build_portfolio_briefing(company_id=company_id, strict_reads=True)
        ...
        logger.warning("[PORTFOLIO] read unavailable: %s", type(exc).__name__)
        raise HTTPException(
            status_code=503,
            detail="Portefeuille indisponible : les points actifs ne peuvent pas être vérifiés.",
        ) from exc
    return {"cards": cards}
