"""Internal deterministic rendering, not ownership or provenance authority.
...
from services.temporal_export import temporal_export_rows
...
def generate_governed_excel(
    ...
    *, metadata: tuple[tuple[str, str], ...], temporal_comparison: dict | None = None,
) -> bytes:
...
def generate_governed_pdf(
    ...
    *, metadata: tuple[tuple[str, str], ...], temporal_comparison: dict | None = None,
    ...
    metadata = [
        ...
        *[[p(label, small), p(value, small)] for label, value in metadata],
...
def generate_governed_pptx(
    ...
    *, metadata: tuple[tuple[str, str], ...], temporal_comparison: dict | None = None,
    ...
    disclosure = dict(metadata)
    add_text(cover, f"Periode {source.current_period or 'UNKNOWN'} | {disclosure['Perimetre']}",
             left=0.78, top=3.9, width=8.5, height=0.45, size=16, text_color=_TEXT)
    add_text(cover, f"Analyse {analysis_id} | {disclosure['Fournisseur']} | {disclosure['Reseau externe']}",
    ...
    if len(metadata) > 3:
        paginate("Provenance d'execution", [f"{label} | {value}" for label, value in metadata])
