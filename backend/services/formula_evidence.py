...
SAME-ARTIFACT GUARANTEE (implementation mission §5): `load_formula_workbook`
takes `file_bytes: bytes` — the exact same parameter shape as
`file_parser.parse_file(file_bytes, filename)` (verified against
backend/services/file_parser.py:35 and backend/connectors/file_connector.py's
FileConnector, which reads `UploadFile` bytes exactly once in
backend/routers/analyze.py and reuses that same `bytes` object for both the
quality gate and the parse). Any caller holding that same `bytes` object can
pass it to both the canonical numeric parse and this module with no
re-download, no alternate path, and no regenerated copy — both reads are
`io.BytesIO(file_bytes)`-wrapped views of the identical byte string, exactly
mirroring file_parser.py's own internal `io.BytesIO(file_bytes)` usage
