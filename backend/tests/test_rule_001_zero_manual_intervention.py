...
class TestPPTXContent:
    ...
    def test_pptx_has_20_slides(self, generated_files):
        # S10 (Bridge historique) + S11 (Bridge financier) + S13 (Création de valeur) ajoutées → 20 slides
        prs = Presentation(str(generated_files["pptx"]))
        assert len(prs.slides) == 20, (
            f"RULE 001 VIOLATION — PPTX : {len(prs.slides)} slides, attendu 20."
        )
